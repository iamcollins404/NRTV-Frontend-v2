...
function Login() {
  ...
  const formSubmit = (e) => {
    ...
    e.preventDefault();
    axios.post("https://hammerhead-app-4pz5b.ondigitalocean.app/auth/login",
      {
        phoneNumber,
        password,
      },
      ...
      .then(function (response) {
        seterror("")
        setSuccessMessage("Successfully logged in. Redirecting you now!")
        ...
        <div className="overflow-hidden bg-white rounded-md shadow-md">
          <div className="px-4 py-6 sm:px-8 sm:py-7">
            <div className="text-center">
              <h5 className="text-3xl font-bold text-gray-900">Please Login to Continue</h5>

              <p className="mt-2 text-base text-gray-600">Don't Have an Account?
                <Link to="/auth/signup">
                  <p className="text-blue-600 transition-all duration-200 hover:underline hover:text-blue-700">Signup Now for Free!</p>
                </Link>
              </p>
            </div>
            ...
            <form className="mt-5" onSubmit={formSubmit}>
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="mt-2.5">
                    <input type="text" value={phoneNumber}
                      onChange={(e) => setphoneNumber(e.target.value)} placeholder="Enter phone number to get started" className="block w-full p-4 text-black placeholder-gray-500 
                    transition-all duration-200 bg-white border border-gray-200 rounded-md focus:outline-none focus:border-blue-600 caret-blue-600" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2.5">
                    <input type="password" placeholder="Enter your password" value={password}
                      onChange={(e) => setpassword(e.target.value)} className="block w-full p-4 
                     text-black placeholder-gray-500 transition-all duration-200 bg-white border 
                    border-gray-200 rounded-md focus:outline-none focus:border-blue-600 caret-blue-600" />
                  </div>
                </div>

                <div className="text-center">
                  <p className="mt-2 text-base text-gray-600">Forgot Account Password?
                    <Link to="/auth/forgotpassword">
                      <p className="text-blue-600 transition-all duration-200 hover:underline hover:text-blue-700">Click Here to Reset!</p>
                    </Link>
                  </p>
                </div>
                ...
                {
                  !processing && <div>
                    <button type="submit" className="inline-flex items-center justify-center w-full px-4 py-4 text-base font-semibold text-white transition-all duration-200 bg-blue-600 border 
                  border-transparent rounded-md focus:outline-none hover:bg-blue-700 focus:bg-blue-700">Proceed Log In</button>
                  </div>
                  || <LoadingButton />
                }

                <div>
                  <button type="button" className=" relative inline-flex items-center justify-center w-full px-4 py-4 text-base font-semibold text-gray-700 transition-all 
                  duration-200 bg-white border-2 border-gray-200 rounded-md
